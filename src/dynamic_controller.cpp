...
// USE_LOG is currently not implemented
// USE_LOG 0: use data freshly collected
// USE_LOG 1: use logged data as goal
#define USE_LOG 0
...
public:
  ...
  Controller(
    ...
    , trjx()
#endif
    , GaoModel("/home/sun/Downloads/rmuAnyVelWeightRDU_7_500_317.json")
    , weight(get(n, "Aggs"))
    , m_dynamicGoalPos()
...
private:
  ...
  void dynamicTrackingSwitch(const geometry_msgs::Vector3& msg)
  {
    ...
    else
    {
      ...
      logFile.open("/home/sun/Desktop/df/log.csv");
      logEventFile.open("/home/sun/Desktop/df/log_event.csv");
      //ROS_ERROR("Here!!");
    }
  }
    ...
    std_srvs::Empty::Response& res)
  {
    ...
    else
    {
      ROS_ERROR("Dynamic tracking is on!");
      logFile.open("/home/sun/Desktop/df/log.csv");
      logEventFile.open("/home/sun/Desktop/df/logEventFile.csv");
    }
    ...
    case Flying:
    {
      ...
      else if (m_flyingState == Following)
      {
        ...
        else
        {
          ...
          // plan new trajectory
          if (newDynamicGoalAwaits == true && ros::Time::now().toSec() - lastTImeNewTrajectory > lastPlanningHorizon * 0.5)
          {
            double timeBeforeReplan = ros::Time::now().toSec();
#if TRJ == 2
            // generate a new trajectory
            // double xin[7] = { -(this->dynamicGoalPx - X), -(this->dynamicGoalPy - Y), -(this->dynamicGoalPz - Z), Vx, Vy, Vz, log10(weight)}; // initial point
            // set target to current target plus one second later
            double predictTime = 0.5 / distanceToGoal;
            double xin[7] = { -(this->dynamicGoalPx + dynamicGoalVx * predictTime - X), -(this->dynamicGoalPy + dynamicGoalVy * predictTime  - Y), -(this->dynamicGoalPz + dynamicGoalVz * predictTime * 0 - Z), Vx, Vy, Vz, log10(weight)}; // initial point
            VX vec = GaoModel.evalQP(xin);
            MX acce = acceGen(vec);
            trj.generateTrajectory(vec, acce);
            trj.setPositionOffset(X - xin[0], Y - xin[1], Z - xin[2]);

            // ROS_ERROR("distance: %f, time: %f, vel: %f", distanceToGoal, trj.getDuration(), snapVelocity);
#elif TRJ == 3
            double predictTime = 0.5 / distanceToGoal;
            double xin[7] = { -(this->dynamicGoalPx + dynamicGoalVx * predictTime - X), -(this->dynamicGoalPy + dynamicGoalVy * predictTime  - Y), -(this->dynamicGoalPz + dynamicGoalVz * predictTime * 0 - Z), Vx, Vy, Vz, log10(weight)}; // initial point
            VX vec = GaoModel.evalQP(xin);
            MX acce = acceGen(vec);
            trjx.generateTrajectory(vec, acce);
            trjx.setPositionOffset(X - xin[0], Y - xin[1], Z - xin[2]);

            // lastPlanningHorizon = trjx.getDuration();

            double wayPointsX[2] = {X, dynamicGoalPx + dynamicGoalVx * predictTime};
            double wayPointsY[2] = {Y, dynamicGoalPy + dynamicGoalVy * predictTime};
            double wayPointsZ[2] = {Z, dynamicGoalPz + dynamicGoalVz * predictTime * 0};
            ...
            double snapVelocity = distanceToNextGoal / trjx.getDuration();
            trj.generateTrajectory(wayPointsX, wayPointsY, wayPointsZ, Vx, Vy, Vz, 0, 0, 0, 2, snapVelocity);

            ROS_ERROR("distance: %f, time: %f, snap vel: %f", distanceToNextGoal, trjx.getDuration(), snapVelocity);

#endif
            lastPlanningHorizon = trj.getDuration();

            newDynamicGoalAwaits = false;
            trajectoryStartTime = ros::Time::now().toSec();
            lastTImeNewTrajectory = trajectoryStartTime;

            ROS_INFO("Replan duration: %f", trajectoryStartTime - timeBeforeReplan);

            logEventFile << trajectoryStartTime - StartTime << ", " << 1 << ", " << dynamicGoalPx << ", " << dynamicGoalPy << ", " << dynamicGoalPz << ", " << endl;
          }
        ...
        logFile << ros::Time::now().toSec() - StartTime << ", " << m_goal.pose.position.x << ", " << m_goal.pose.position.y << ", " << m_goal.pose.position.z << ", " << X << ", " << Y << ", " << Z << ", " << dynamicGoalPx << ", " << dynamicGoalPy << ", " << dynamicGoalPz << ", " << dynamicGoalVx << ", " << dynamicGoalVy << ", " << dynamicGoalVz << endl;
...
private:
  ...
  ofstream logFile;
  ofstream logEventFile;

  double lastPlanningHorizon;
